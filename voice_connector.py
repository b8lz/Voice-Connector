...
class VoiceConnector:
        ...
        while True:
            try:
                ...
                async with websockets.connect(uri, max_size=None) as self.websocket:
                    ...
                    self.heartbeat_task = asyncio.create_task(self.send_heartbeats(heartbeat_interval))
                ...
                if self.reconnect_attempts > self.max_reconnect_attempts:
                    await self.print_status("Max reconnection attempts reached.", "error")
                    return

                retry_in = min(CONFIG["reconnect_delay"] * (2 ** (self.reconnect_attempts - 1)), 300)
                await self.print_status(f"Connection closed. Reconnecting in {retry_in} seconds...", "error")
                ...
                self.reconnect_attempts += 1
                await self.display_banner()
                await self.print_status("Error occurred, reconnecting...", "error")
                await asyncio.sleep(CONFIG["reconnect_delay"])
    ...
    async def run(self):
        try:
            ...
            await self.print_status("Fatal error, restarting in 30 seconds...", "error")
